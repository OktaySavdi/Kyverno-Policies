...
| 5 | `disallow-share-process-namespace` | Deny | |
| 6 | `disallow-secrets-from-env` | Deny | blocks secretKeyRef/secretRef in env |
| 7 | `disallow-unsafe-sysctls` | Deny | kernel safe-sysctl allowlist only |
...
| 9 | `mutate-set-automount-sa-token-false` | injects `automountServiceAccountToken: false` when not defined |

**Total: 24 policies** (7 ValidatingPolicy, 9 MutatingPolicy, 1 strategy doc)
...
Admission request
  ...
  • mutate-set-automount-sa-token-false           (absent)    → false
       │
